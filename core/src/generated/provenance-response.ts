...
export const ProvenanceResponseSchema = z
  .object({
    ...
    claim_kind: z.enum(["numeric", "trend", "substrate_score"]),
    ...
    served_at: z.string().datetime({ offset: true }).describe("When this claim was first emitted."),
    sources: z
      .array(
        z
          .object({
      ...
      .min(1)
      .describe("Deduplicated source observations behind the claim."),
    unit: z.string(),
    validation: z
      .object({
        confidence: z.number().gte(0).lte(1),
    ...
    value_repr: z.string().describe("The value as originally served, rendered as text."),
  })
  .strict()
  .describe("Full trace of a previously served claim back to source observations.");
