/**
 * REST mirror of the MCP surface — the app itself, with no listener.
 *
 * Kept separate from `server.ts` so the same configured app can be mounted inside another
 * runtime. The deployed console mounts it under /api, which is how one Vercel deployment
 * serves both the console and the layer's REST interface without either duplicating the
 * other.
 *
